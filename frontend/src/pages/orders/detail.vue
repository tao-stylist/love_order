  ...
  <view class="page-detail">
    ...
    <view class="section-card">
      ...
      <view class="info-row">
        ...
        <text class="info-value">{{ orderInfo.name || '未填写' }}</text>
      </view>
      <view class="info-row">
        <text class="info-label">电话</text>
        <text class="info-value">{{ orderInfo.phone || '未填写' }}</text>
      </view>
      <view class="info-row">
        <text class="info-label">地址</text>
        <text class="info-value">{{ orderInfo.address || '未填写' }}</text>
      </view>
    </view>
    ...
    <view class="section-card">
      ...
      <view class="detail-items">
        <view v-for="item in orderItems" :key="item.menuId" class="detail-item">
          ...
          <text class="detail-item-price">¥{{ item.totalPrice }}</text>
        </view>
      </view>
      ...
      <view class="price-summary">
        ...
        <view v-if="orderInfo.deductAmount > 0" class="summary-row">
          <text class="summary-label">积分抵扣</text>
          <text class="summary-value discount">-¥{{ orderInfo.deductAmountText }}</text>
        </view>
    ...
    <view class="section-card">
      ...
      <view v-if="orderInfo.deliveryTime" class="info-row">
    ...
    <!-- 评价入口 -->
    <view v-if="orderInfo.status === 'completed'" class="review-entry" @click="goReview">
      <text class="review-entry-text">💕 为这份爱心订单评价</text>
      <text class="review-entry-arrow">></text>
    </view>

    <!-- 底部操作 -->
    <view v-if="orderInfo.status === 'completed'" class="bottom-actions safe-bottom">
...
const orderInfo = computed(() => {
  ...
  return {
    status: data.status || '',
    statusText: data.statusText || '加载中...',
    name: data.name || '',
    phone: data.phone || '',
    address: data.address || '',
    orderNo: data.orderNo || '',
    createTimeText: data.createTime ? formatDate(data.createTime, 'YYYY-MM-DD HH:mm') : '',
    deliveryTime: data.deliveryTime || '',
    remark: data.remark || '',
    deductAmount: data.deductAmount || 0,
    deductAmountText: (data.deductAmount || 0).toFixed(1),
...
const orderItems = computed(() => {
  const items = order.value && order.value.items ? order.value.items : []
  return items.map(item => ({
    ...item,
    totalPrice: (item.price * item.quantity).toFixed(1)
  }))
})
...
const statusBg = computed(() => {
  ...
  return bgMap[orderInfo.value.status] || 'linear-gradient(135deg, #FF6B9D, #FFA07A)'
})

// 小计
const subtotal = computed(() => {
  if (!order.value || !order.value.items) return '0.0'
  const total = order.value.items.reduce((sum, item) => sum + item.price * item.quantity, 0)
  return total.toFixed(1)
})

// 最终价格
const finalPrice = computed(() => {
  const price = orderInfo.value.finalPrice || parseFloat(subtotal.value)
