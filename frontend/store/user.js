...
export const useUserStore = defineStore('user', () => {
  ...
  async function dailySign() {
    ...
    try {
      // 模拟签到接口
      // const res = await post('/user/sign')

      // 模拟签到成功
      const addPoints = Math.floor(Math.random() * 10) + 5 // 随机5-15积分
      lovePoints.value += addPoints
      isSigned.value = true

      // 更新本地存储
      const info = getUserInfo()
      if (info) {
        info.lovePoints = lovePoints.value
        setUserInfo(info)
      }

      uni.showToast({
        title: `签到成功！+${addPoints}爱心积分`,
        icon: 'none',
        duration: 2000
      })
